...
public class CustomAnalogClock extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawHandLine(canvas, (hour + calendar.get(Calendar.MINUTE) / 60) * 5f, true, false, Color.BLACK); // draw hours
        drawHandLine(canvas, calendar.get(Calendar.MINUTE), false, false, Color.BLACK); // draw minutes
        drawHandLine(canvas, calendar.get(Calendar.SECOND), false, true, Color.RED); // draw seconds

        /** invalidate the appearance for next representation of time  */
        postInvalidateDelayed(500);
        invalidate();
    }
    private void drawHandLine(Canvas canvas, double moment, boolean isHour, boolean isSecond, int col) {
        ...
        if (isSecond) mPaint.setColor(col);
